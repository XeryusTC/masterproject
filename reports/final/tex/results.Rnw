...
algorithm OD+ID. To do this we compare them on a large set of problem instances.
The exact size of the problem set depends on the experiment.
...
There are several experiments that are done to compare the proposed algorithms
to the complete method OD+ID. First the weights that influence the evaluation
of proposals made during dialogues need to be set, this is discussed in
\autoref{sec:evalweights}. The main hypothesis is that all proposed algorithms
are able to solve problem instances faster than OD+ID, but none of the
algorithms matches OD+ID in terms of the quality of the solution. The quality
of a solution is measured as the sum of the lengths of the paths that agents
have to take for them to arrive at their destination position. Several
sub-experiments are also conducted such as the dependence on the number of
agents in the conflict to be able to find a solution, and the influence of
caching paths on solution time.
...
agent's plan. To be able to evaluate a proposal properly there are several
weights that need to be set so that each effect is taken into account as much
as it needs to be. To set the weights simulated annealing
...
show results that can be expected in an application while not spending too much
time on very complex instances that require a lot of computation. The results
of the simulated
...
is constructed following the same procedure as used to find the optimal
evaluation weights.
...
the $n$th instance for a different algorithm. The graph shows several pieces of
information. The
...
%so where the graph crosses the top indicates how many of the problems
%were solved by the algorithm.

%The graph in \autoref{fig:perfgraph} shows the performance characteristic for
%the tested algorithms.
The performance graph shows that only the PCA* is generally slower than the
complete algorithm OD+ID. When looking at individual instances  we can see that
$\Sexpr{round(naive.vs.odid * 100, 1)}\%$ of the instances that both PCA* and
OD+ID solved the latter was faster. All other methods are faster than OD+ID and
are able to find a solution to more instances. The DPCA* and DPCA*+ versions of
the algorithm have an almost equal performance. DPCA* is slightly faster on
each individual instance by $\Sexpr{signif(abs(base.vs.plus$estimate * 1000),
2)}$ ms as shown by a one-sided paired t-test
$(t(\Sexpr{base.vs.plus$parameter}) = \Sexpr{round(base.vs.plus$statistic, 2)},
...
1)}\%$ of instances while DPCA*+ is able to solve $\Sexpr{round(solved$time[4]
* 100, 1)}\%$. This indicates that the added complexity in DPCA*+ does not
decrease the time required to solve a problem instance.
...
2)}, p=\Sexpr{signif(t.4vs8$p.value, 3)}$ for $w=8$). This indicates that there
...
$w=2$ and $=8$. This means that the choice of window size barely matters, only
the use of a window to limit communication has an effect on the performance.
...
to solve problems that include more agents. It shows that DPCA* and DPCA*+ have
the same overall
...
either be because an agent has a \emph{wait} action on that node. It can also
...
agents to move through. These loops are expected behaviours that occur when
agents cooperate. We also do not count \emph{wait} actions as loops, so only
the number of unique times that an agent visits a node are counted.
...
in the solutions provided by OD+ID and other versions of the algorithm. They
...
that agents have a larger view of the world and are better able to create a
solution where agents get closer to their goal without having to go back. When
...
are more dialogues that take place. This is to be expected because agents do
not start a dialogue for every conflict on the most optimal path, but only for
those conflicts that occur within $w$ time steps. After they solve these
conflicts they execute
$\sfrac{w}{2}$ steps of the plan and move the centre of the window to their new
position. Next they start resolving any conflicts that occur within the updated
...
but at some point this trend trails off. This point lies between 25 and 30
agents depending on the algorithm. There is a peak in the number of dialogues
between 28 and 35 agents. This is likely to be because the algorithms are only
able to find a solution to problems with few conflicts. More complex problems
with many agents would require more dialogues but these can't be completed
within the 2000ms time limit. We can see that the point where the growth in the
number of  dialogues required to solve an instance starts to increase coincides
with \autoref{fig:solved}. The start of the steep cliff for each respective
...
%It shows how many dialogues have $n$ agents
%trying to find a solution to a conflict depending on the total number of agents
%in a problem instance.
It shows that in most dialogues only two agents participate. It is not a given
...
#lines(seq(length(plus.cache)), plus.cache, col=color.set.cache[2], lty=1)
lines(seq(length(window2.cache)), window2.cache, col=color.set.cache[3], lty=1)
#lines(seq(length(window4.cache)), window4.cache, col=color.set.cache[4], lty=1)
...
tested.
The solid lines in the plot represent the
...
employed.
There is also an interaction between the algorithm and whether the cache is
enabled $(F(\Sexpr{cache.sum[['Df']][4]}, \Sexpr{cache.anova$df.residual}) =
\Sexpr{round(cache.sum[['F value']][4], 2)},
p=\Sexpr{signif(cache.sum[['Pr(>F)']][4], 3)})$. This confirms what the
performance graph in \autoref{fig:cache} shows, the effectiveness of the cache
depends on the algorithm that is used. Using no window or a large window
benefit more from employing a cache than when a small window is used.