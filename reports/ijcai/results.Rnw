...
quality paths. We expect that our proposed algorithms perform on an equal 
...
overfitting. The results of annealing are shown 
in \autoref{tbl:annealing}. The
...
\emph{performance graph}~\cite{standley2010} as shown in 
\autoref{fig:perfgraph}. The $x$-axis
...
One thing that is noticeable is that the number of dialogues required
...
The agents make use of a path cache to reduce the amount computation to 
resolve 
conflicts. If the cache is in use then there should be a noticeable decrease 
in 
the time it takes to solve problems. To measure the effect of caching on 
solving cooperative pathfinding problems we conducted a separate experiment of 
1000 problem instances. All other parameters were the same as in previous 
experiments. DPCA* solved each instance with the path cache enabled and 
disabled. The result of this experiment is shown in \autoref{fig:cache}. We do 
