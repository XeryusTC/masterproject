...
resolve this conflict to prevent the collision.
...
To allow agents to evaluate a proposal we we define $\vote_{a_n}(a_i > a_j)$ as 
...
they want to make further proposals:
...
PCA* and DPCA* both find the same solution as shown in \autoref{fig:pca-5} and 
\autoref{fig:example-solution}. During the calculation of a solution PCA* did 
not take any new conflicts into account. Because of this it picked a solution 
to the initial conflict between $a_1$ and $a_2$ that DPCA* would have 
considered a worse solution. We can see why DPCA* has settled on the solution 
in \autoref{fig:example-solution} while the result of PCA* is determined by how 
ties between equal priority orderings are settled. The dialogues in DPCA* add 
transparency to the process of finding a solution.
