...
agents. \autoref{tbl:quality} paints a similar picture with regards to the 
number of instances solved. DPCA* and DPCA*+ are able to solve double the 
amount of instances as OD+ID and PCA*. DiMPP falls in the middle of these two 
groups in both comparisons. In general DPCA* and DPCA*+ find longer paths than 
OD+ID but they are also able to solve instances with more agents for which the 
sum path length is longer by virtue of there being more agents with paths. A 
better comparison is given in \autoref{fig:lengths}. This shows that PCA* finds 
the shortest paths followed by OD+ID. DPCA* and DPCA*+ find longer paths while 
DiMPP causes the largest detours.
