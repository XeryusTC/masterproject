...
Computational argumentation has been used to construct a plan for an agent,
this field of argumentation is known as \emph{practical reasoning}. It has
also been used to make plans in a multi-agent system, allowing those agents to
coordinate. A successful resolution to the cooperative pathfinding problem also
requires that agents are able to coordinate. Argumentation has not yet been
used to find solutions for cooperative pathfinding, but research in
argumentation has been generic enough that it can be applied to a specific
field as cooperative pathfinding. Relevant work in cooperative pathfinding,
argumentation and coordination is discussed in this section.
...
\begin{table}[b]
    ...
    Communication indicates what limit is imposed on which agents can
    communicate, `all' means that there is no limit and agents can communicate
    with all others. Online enumerates whether all movement actions for all
    agents are determined before execution, some algorithms update the plan
    during execution.}
    ...
    \begin{tabular}{l|l|l|l|l|l}
        ...
        WHCA* \cite{silver2005} & Decoupled & No & Yes\footnotemark[1] & Window
        & Yes \\
