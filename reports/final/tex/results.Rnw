...
implemented in Python 3.6 and were run on an AMD FX-8120 clocked at 3.1GHz.
...
window also means that instances with fewer agents can't always be solved. The
...
<<dialogues,echo=F,cache=T,cache.beater=conflicts.aggr,dev='tikz',fig.width=5,fig.height=2,fig.align='center',fig.pos='b',fig.cap=conflicts.title>>=
op <- par(mar = c(4, 3.8, 1, 1))
plot(1,
    type='l',
    xlim=c(0,40),
...
\autoref{fig:conflict-sizes}. It shows what the number of agents in each
dialogue is depending on the number of agents in the conflict in total. It
...
dialogues in which two agents try to solve a conflict. DPCA*+ often doesn't
...
this 1000 problem instances were generated. Each of the (W)DPCA* algorithms
were presented with each instance while the cache was turned off and turned on.
Algorithms were given 2000ms to solve each instance as before, this results in
