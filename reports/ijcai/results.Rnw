...
%
%legend("bottomright",
%       legend=cannonical.names.cache[1],
%       col=color.set.cache[1],
%       pch=16,
%       bty="n", cex=.8)
%@
%
...
%The figure shows that after the initial 100 problem instances the path cache 
%does decrease the time required to find a solution. DPCA* is also able to 
%solve 
%more problem instances when the path cache is enabled.