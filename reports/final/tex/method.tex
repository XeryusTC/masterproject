...
resources.
%The algorithm requires agents to resolve all conflicts before executing the
%plan, requiring that all the work is done up front. It is also possible to
%limit by only resolving those conflicts that happen within a certain time
%window. Any conflicts outside of the window are ignored until the agent has
%moved closer. During most of the execution there is only a partial solution
%available to the cooperative pathfinding problem, but this partial solution is
%continuously updated until a full solution is found.
...
%execute a solution. Computational effort is also decreased by virtue of the
%paths being shorter. Because agents will have moved before the next set of
%conflicts will be solved they have moved closer to their goal. This means that
%the depth of the search is not as large and less computation is required to
%find conflict free paths for the new window.
