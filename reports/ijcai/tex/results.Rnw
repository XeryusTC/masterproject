...
algorithm OD+ID and the decentralized algorithm DiMPP. To do this we compare
them on a large set of problem instances.
The exact size of the problem set depends on the experiment.
Each instance in the set consists of a $16 \times 16$ 8-connected grid. Each
grid cell has
...
to OD+ID and DiMPP. First the weights that determine the evaluation of
proposals made during dialogues need to be set. The empirically found weights
...
solutions. The quality of a solution is primarily quantified as the sum of the
lengths of the paths that agents have to take to arrive at their destination
position. The number of loops that occur within an agent's path is used as a
secondary measurement of the quality of a solution. We expect that the proposed
methods perform on an equal or slightly worse level to DiMPP on all of these
...
find a solution solution is also recorded. This is not compared to OD+ID or
...
annealing \citep{kirkpatrick1983} is used with an initial temperature of 100
...
<<perfgraph,sanitize=T,echo=F,warning=F,cache=T,cache.beater=dat,dev='pdf',fig.height=2,fig.width=5,fig.align='center',fig.pos='t',fig.cap=perfgraph.title>>=
...
algorithm. The graph shows several different pieces of information. The
...
using a window to limit communication itself has a larger effect on the
performance of DPCA*.
%The window size has a large effect on the sum of the path lengths of the
%agents as shown in \autoref{tbl:length}.

<<solved,echo=F,cache=T,cache.beater=solved.aggr,dev='pdf',fig.width=5,fig.height=2,fig.align='center',fig.pos='t',fig.cap=solved.title>>=
...
results in a larger sum of the path lengths for a solution. This can also be
...
<<lengths,echo=F,warning=F,cache=T,cache.beater=dat.lengths,dev='pdf',fig.width=5,fig.height=2,fig.align='center',fig.pos='t',fig.cap=lengths.title>>=
op <- par(mar = c(3.8, 3.8, 1, 1))
plot(c(),
     type='n',
     xlim=c(5,35),
...
<<dialogues,echo=F,cache=T,cache.beater=conflicts.aggr,dev='pdf',fig.width=5,fig.height=2,fig.align='center',fig.pos='t',fig.cap=conflicts.title>>=
op <- par(mar = c(4, 3.8, 1, 1))
plot(1,
    type='l',
    xlim=c(0,35),
...
number of dialogues required rises exponentially when the number of agents
increases, but at some point this trend trails off. This point lies between 25
and 30 agents depending on the algorithm. There is a peak in the number of
dialogues between 28 and 35 agents.

<<conflict-sizes,echo=F,cache=T,cache.beater=sizes.aggr,dev='pdf',fig.width=5,fig.height=2,fig.align='center',fig.pos='t',fig.cap=sizes.title>>=
op <- par(mar = c(4, 3.8, 1, 1))
plot(1,
    type='n',
    xlim=c(1,35),
...
<<cache,echo=F,warning=F,cache=T,cache.beater=dat.cache,dev='pdf',fig.width=5,fig.height=2,fig.align='center',fig.pos='t',fig.cap=cachegraph.title>>=
op <- par(mar = c(3.8, 3.8, 1, 1))
plot(c(), c(),
    type='l',
    log='y',
...
legend("bottomright",
    legend=cannonical.names.cache[c(1,2)],
    col=color.set.cache[c(1,2)],
    pch=16,
    bty="n", cex=.8)
@
