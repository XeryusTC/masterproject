...
\author[a]{\fnms{Xeryus} \snm{Stokkel}
\thanks{Corresponding Author: \texttt{brontitall@dds.nl}}},
\author[a]{\fnms{Bart} \snm{Verheij}}

\runningauthor{Stokkel and Verheij}
\address[a]{University of Groningen}

%\maketitle

\input{tex/abstract}
\begin{keyword}
Deliberation dialogues\sep cooperative pathfinding\sep multi-agent system\sep 
practical reasoning\sep partial global planning
\end{keyword}
\end{frontmatter}
